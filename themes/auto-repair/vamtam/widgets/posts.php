...
class wpv_posts extends WP_Widget {
	...
	public function wpv_posts() {
		$widget_options = array(
			'classname' => 'wpv_posts',
			'description' => __( "Displays a list of posts/comments", 'auto-repair' )
		);
		$this->WP_Widget( 'wpv_posts', __( 'Vamtam - Multi widget', 'auto-repair' ) , $widget_options );
	...
	private function get_section_title( $orderby, $single=false ) {
		if ( $orderby == 'comment_count' )
			return apply_filters( 'wpv_multiwidget_tab_title', __( 'Popular', 'auto-repair' ), $orderby, $single );
		if ( $orderby == 'date' )
			return apply_filters( 'wpv_multiwidget_tab_title', __( 'Newest', 'auto-repair' ), $orderby, $single );
		if ( $orderby == 'comments' )
			return apply_filters( 'wpv_multiwidget_tab_title', __( 'Comments', 'auto-repair' ), $orderby, $single );
		if ( $orderby == 'tags' )
			return apply_filters( 'wpv_multiwidget_tab_title', __( 'Tags', 'auto-repair' ), $orderby, $single );
	}
