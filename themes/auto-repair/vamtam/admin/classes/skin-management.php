...
class WpvSkinManagement extends WpvAjax {
	...
	public function available_skins() {
		...
		if ( isset( $_POST['prefix'] ) ) {
			...
			echo '<option value="">' . __( 'Available skins', 'auto-repair' ) . '</option>'; // xss ok
	...
	public function load_skin() {
	  ...
	  array(
	  'data' => $data,
	  'text' => '<span class="success">' . __( 'The skin has been imported successfully. Have fun!', 'auto-repair' ) . '</span>',
	  )
		); // xss ok
	...
	public function delete_skin() {
		...
		if ( @unlink( WPV_SAVED_OPTIONS . $_POST['file'] ) ) {
			echo '<span class="success">'. __( 'Success.', 'auto-repair' ) . '</span>'; // xss ok
			exit;
		}

		echo '<span class="error">'. __( 'Cannot delete file.', 'auto-repair' ) . '</span>'; // xss ok
		exit;
	}
	...
	public function save_skin() {
		...
		echo '<span class="error">'. __( 'Cannot save file.', 'auto-repair' ) . '</span>'; // xss ok
		exit;
	}
